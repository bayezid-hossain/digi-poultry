...
import { FCRStandards, FCRTable, cycles, notifications, userCycle } from "@/server/db/schema";
...
export async function createFCR(
  ...
  if (user && session.organization) {
    ...
    const isEligible = await db
      ...
      .where(
        and(
          ...
          eq(userCycle.orgId, session.organization),
    ...
    if (isEligible[0]) {
      ...
      return { error: "You don't belong to this cycle" };
    }
  } else {
    return { error: "Not a part of an organization" };
  }
}
