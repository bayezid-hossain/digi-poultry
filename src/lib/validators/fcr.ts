...
export const feedSchema = z.object({
  name: z
    .string({
      ...
      invalid_type_error: "Feed Name must be string",
    })
    .min(1, "Feed Name is required"),
  quantity: z
    .number({
      required_error: "Feed Quantity is required",
      invalid_type_error: "Feed Quantity must be a number",
    })
    .gt(-1, { message: "Cannot be (-)" }),
});
export const fcrSchema = z.object({
  ...
  farmStock: z.array(feedSchema, {
    required_error: "Farm Stock Required",
    invalid_type_error: "Invalid Farm Stock Data",
  }),
});
...
export const updateSingleRowSchema = singleStandardSchema.extend({
  previousAge: z
    ...
    .gt(-1, { message: "Previous Age must be greater than 0" }),
});
